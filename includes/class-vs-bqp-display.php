...
class VS_BQP_Display {
    ...
    public function enqueue_assets() {
        if ( is_product() ) wp_enqueue_script( 'vs-bqp-frontend', plugins_url( 'assets/js/frontend.js', VS_BQP_FILE ), array( 'jquery', 'wc-add-to-cart-variation' ), VS_BQP_VERSION, true );
    }
    ...
    private function variable_has_boxed_variations( $product ) {
        foreach ( $product->get_children() as $variation_id ) {
            ...
            if ( $variation && $this->product_data->get_units_per_box( $variation ) > 1 ) return true;
        }
        return false;
    }
}
